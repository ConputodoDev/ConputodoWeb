...
export default function StoreFront() {
  ...
  const [currentView, setCurrentView] = useState(() => {
    return localStorage.getItem('conputodo_view') || 'home';
  });
  
  const [viewProduct, setViewProduct] = useState(() => {
    const saved = localStorage.getItem('conputodo_product');
  ...
  // --- LÓGICA DE FILTRADO EN CASCADA (IMPORTANTE) ---
  ...
  // --- MANEJO DE CHECKBOXES Y ACORDEONES ---
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-slate-800 relative">
      ...
      {currentView !== 'product-detail' && <Footer onNavigate={setCurrentView} onCatalog={() => goToCatalog()} />}
    </div>
  );
}
    ...
    return (
        <div className="container mx-auto px-4 py-16 max-w-4xl animate-in fade-in duration-500">
            ...
            <div className="bg-white p-12 rounded-2xl shadow-sm border border-gray-100 space-y-10">
                ...
                <div className="space-y-4"><h3 className="text-xl font-bold">Privacidad</h3><p className="text-slate-600">Tus datos solo se usan para facturación y envío.</p></div>
            </div>
        </div>
    ); 
}
      ...
      <div className="container mx-auto px-4 py-16 grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="space-y-8">
            ...
            <div className="w-full h-80 rounded-2xl overflow-hidden border border-gray-200 shadow-inner relative z-0">
               <MapContainer center={[10.249758418955139, -67.60040315522524]} zoom={17} scrollWheelZoom={false} style={{ height: "100%", width: "100%" }}><TileLayer attribution='© OSM' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/><Marker position={[10.249758418955139, -67.60040315522524]}><Popup>Conputodo</Popup></Marker></MapContainer>
            </div>
        </div>
    ...
    <div className={`fixed inset-0 z-50 bg-black/50 flex justify-end transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={onClose}>
      <div className={`w-full max-w-md bg-white h-full shadow-2xl flex flex-col transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {step === 'cart' && (cart.length === 0 ? <div className="text-center py-10 text-slate-400">Tu carrito está vacío</div> : cart.map(item => <div key={item.id} className="border-b py-2 flex justify-between"><span>{item.title} (x{item.qty})</span> <button onClick={() => onRemove(item.id)} className="text-red-500"><Trash2 size={16}/></button></div>))}
...
function Footer({ onNavigate, onCatalog }) {
    return (
        <footer className="bg-neutral-900 text-white pt-16 pb-8 border-t border-neutral-800 mt-auto">
            <div className="container mx-auto px-4">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
                    ...
                    <div><h4 className="font-bold text-lg mb-6 border-b border-neutral-800 pb-2 inline-block">Navegación</h4><ul className="space-y-3 text-sm text-neutral-400"><li><button onClick={() => onNavigate('home')} className="hover:text-[#FF6600] transition-colors flex items-center gap-2"><ArrowRight size={14}/> Inicio</button></li><li><button onClick={onCatalog} className="hover:text-[#FF6600] transition-colors flex items-center gap-2"><ArrowRight size={14}/> Catálogo</button></li><li><button onClick={() => onNavigate('contact')} className="hover:text-[#FF6600] transition-colors flex items-center gap-2"><ArrowRight size={14}/> Contacto</button></li><li><button onClick={() => onNavigate('legal')} className="hover:text-[#FF6600] transition-colors flex items-center gap-2"><ArrowRight size={14}/> Legales</button></li></ul></div>
