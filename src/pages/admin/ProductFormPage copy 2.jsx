...
const ProductFormPage = ({ productToEdit, onBack }) => {
  ...
  // Variantes (Solo si isVariable = true)
  const [variants, setVariants] = useState([]);
  // UI Auxiliar para nueva variante
  ...
  // --- MANEJO DE VARIANTES ---
  const addVariant = () => {
    if (!newVariant.name || !newVariant.price) return alert("Nombre y Precio son obligatorios para la variante");
    ...
    setNewVariant({ name: '', price: '', stock: 1, sku: '' }); // Reset form
  };
  ...
  // --- IA GENERATORS (Misma lógica previa) ---
  ...
  const handleSave = async () => {
    ...
    if (isVariable) {
        if (variants.length === 0) return alert("⚠️ Agrega al menos una variante si el producto es Variable.");
    } else {
        if (!formData.priceUsd) return alert("⚠️ Completa el Precio USD para producto simple.");
    }
    ...
    try {
      ...
      if (isVariable) {
         ...
         // Calculamos totales y mínimos para la vista rápida
         ...
         finalStock = formData.inStock ? 10 : 0; // Si el user no pone stock exacto, usamos bandera (o podrías agregar input stock exacto)
         // Nota: En tu form original no había input de stock numérico exacto para simple, solo el toggle inStock
         // Podríamos asumir un stock genérico o agregar ese campo. Por ahora uso lógica existente + variante.
         if(formData.inStock) {
             // Si el form original no tiene input de numero, asumimos infinito o valor dummy
             // Pero productService espera numero. 
             // Ajuste rápido: Si es simple, usamos el precio del form. El stock lo manejamos con la bandera inStock visualmente,
             // pero en DB guardamos un numero alto si está en stock.
             finalStock = 999; 
         } else {
             finalStock = 0;
         }
      ...
      const productData = {
        ...
        prices: { usd: finalPrice }, // Precio raíz
        stock: finalStock,           // Stock raíz (suma si es variable)
        category: formData.category,
        brand: formData.brand,
        sku: formData.sku,
        inStock: formData.inStock, // Bandera manual
        isFeatured: formData.isFeatured,
        tags: formData.tags,
        specs: formData.specs.map(s => ({ key: s.key, value: s.value })),
        seo: formData.seo,
        images: { main: mainImageUrl, gallery: galleryUrls },
        variants: finalVariants, // Array vacío si es simple
        updatedAt: new Date(),
    ...
    <div className="pb-12 animate-in slide-in-from-right duration-300">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-neutral-500 font-medium">
                {isVariable ? 'Modo: Producto con Variantes' : 'Modo: Producto Simple'}
            </span>
          </div>
        </div>
        <button 
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* 1. TIPO DE PRODUCTO Y DATOS BÁSICOS */}
          <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-6 space-y-5">
            {/* Switch Simple vs Variable */}
            <div className="flex p-1 bg-neutral-100 rounded-lg mb-4 w-fit">
                <button 
                    onClick={() => setIsVariable(false)}
                    className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${!isVariable ? 'bg-white text-[#FF6600] shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}
                >
                    <Box size={16} className="inline mr-2 mb-0.5"/> Producto Simple
                </button>
                <button 
                    onClick={() => setIsVariable(true)}
                    className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${isVariable ? 'bg-white text-[#FF6600] shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}
                >
                    <Layers size={16} className="inline mr-2 mb-0.5"/> Producto Variable
                </button>
            </div>
          ...
          {/* 2. GESTOR DE VARIANTES (SOLO SI ES VARIABLE) */}
          {isVariable && (
            <div className="bg-white rounded-xl border border-orange-200 shadow-sm p-6 animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="font-semibold text-neutral-800 text-lg flex items-center gap-2">
                        <Layers size={20} className="text-[#FF6600]"/> Variantes y Combinaciones
                    </h3>
                </div>
                ...
                <div className="border border-neutral-200 rounded-lg overflow-hidden mb-4">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-neutral-50 text-neutral-500 font-semibold uppercase text-xs">
                            <tr>
                                <th className="p-3">Nombre / Combinación</th>
                                ...
                                <tr key={variant.id} className="group hover:bg-neutral-50">
                                    <td className="p-2">
                                        <input 
                                            ...
                                            className="w-full bg-transparent outline-none font-medium text-neutral-800"
                                        />
                                    </td>
                                    <td className="p-2">
                                        <input 
                                    ...
                                    <td className="p-2">
                                        <input 
                                            ...
                                            className="w-full bg-transparent outline-none text-neutral-600"
                                        />
                                    </td>
                                    <td className="p-2">
                                        <input 
                                            ...
                                            className="w-full bg-transparent outline-none text-xs font-mono text-neutral-500 uppercase"
                                        />
                                    </td>
                                    <td className="p-2 text-center">
                                        <button onClick={() => removeVariant(variant.id)} className="text-neutral-300 hover:text-red-500 transition-colors">
                                            <Trash2 size={16}/>
                                        </button>
                                    </td>
                                </tr>
                            ...
                            <tr className="bg-orange-50/50">
                                <td className="p-2">
                                    <input 
                                        ...
                                        className="w-full px-2 py-1 bg-white border border-orange-200 rounded focus:border-orange-500 outline-none text-sm"
                                    />
                                </td>
                                <td className="p-2">
                                    <input 
                                ...
                                <td className="p-2">
                                    <input 
                                        ...
                                        className="w-full px-2 py-1 bg-white border border-orange-200 rounded focus:border-orange-500 outline-none text-xs font-mono uppercase"
                                    />
                                </td>
                                <td className="p-2 text-center">
                                    <button onClick={addVariant} className="bg-[#FF6600] text-white p-1.5 rounded hover:bg-orange-700 transition-colors">
                                        <Plus size={16}/>
                                    </button>
                                </td>
                            </tr>
                ...
                <p className="text-xs text-neutral-400">
                    * El precio principal del producto será el de la variante más económica. El stock será la suma de todas.
                </p>
            </div>
          )}
          ...
          {/* 4. FICHA TÉCNICA (GLOBAL) */}
          <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-semibold text-neutral-800 text-lg">Ficha Técnica (Global)</h3>
              <button 
        ...
        <div className="space-y-6">
          ...
          {/* PRECIO Y STOCK (SOLO VISIBLE SI ES SIMPLE) */}
          {!isVariable && (
            <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-6 animate-in fade-in duration-300">
                <h3 className="font-semibold text-neutral-800 text-lg mb-4 flex items-center gap-2">
                <DollarSign size={20} className="text-green-600"/> Precio & Stock
                </h3>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-neutral-100 mt-4">
                   <span className="text-sm font-medium text-neutral-700">Disponible (Stock)</span>
                   <button 
          ...
          {/* MENSAJE SI ES VARIABLE EN SIDEBAR */}
          {isVariable && (
            <div className="bg-orange-50 rounded-xl border border-orange-200 p-6 text-center text-sm text-orange-800">
                <Layers size={32} className="mx-auto mb-2 text-orange-400"/>
                <p className="font-bold mb-1">Modo Variable Activado</p>
                <p className="opacity-80 text-xs">Gestiona los precios y el stock individualmente en la tabla de variantes.</p>
            </div>
          )}
          ...
          {/* INFO EXTRA (SKU GLOBAL) */}
          <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-6">
             <h3 className="font-semibold text-neutral-800 mb-4 text-sm uppercase tracking-wider">Identificación</h3>
             <div className="space-y-4">
                <div>
                   <label className="text-xs text-neutral-500 block mb-1 font-bold">SKU General</label>
