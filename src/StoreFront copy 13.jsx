...
import { 
  ...
  Instagram, Facebook, Video, ChevronDown, ChevronUp, Filter
} from 'lucide-react';
...
export default function StoreFront() {
  ...
  // Estado para Acordeones (Spoilers)
  const [openFilters, setOpenFilters] = useState({ brands: true, tags: false });
  ...
  // --- DETECTOR CLICK FUERA DEL BUSCADOR ---
  useEffect(() => {
  ...
  // 1. Base: Productos filtrados por Categoría y Buscador
  ...
  // 2. Opciones Disponibles: Extraemos marcas/tags SOLO de la base filtrada
  ...
  // 3. Final: Filtramos la base por las checkboxes seleccionadas
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-slate-800 relative">
      ...
      <header className="bg-white sticky top-0 z-40 shadow-sm border-b border-gray-100">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex-1 max-w-xl relative hidden sm:block group" ref={searchRef}>
            ...
            {/* Animación del Dropdown de Búsqueda */}
      ...
      {/* --- VISTA: CATÁLOGO CON FILTROS DINÁMICOS --- */}
      {currentView === 'catalog' && (
        ...
        <footer className="bg-neutral-900 text-white pt-16 pb-8 border-t border-neutral-800 mt-auto">
            <div className="container mx-auto px-4">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
                    <div className="md:col-span-1 space-y-4"><div className="flex items-center gap-2 text-white font-bold text-2xl"><div className="w-10 h-10 bg-[#FF6600] rounded-lg flex items-center justify-center">C</div> Conputodo</div><p className="text-neutral-400 text-sm leading-relaxed">Tu aliado tecnológico de confianza en Maracay. Especialistas en computación, redes, energía y Gaming.</p><div className="pt-4 flex items-center gap-2 text-neutral-500 text-xs"><MapPin size={16}/> Maracay, Edo. Aragua</div></div>
                    ...
                    <div><h4 className="font-bold text-lg mb-6 border-b border-neutral-800 pb-2 inline-block">Conputodo</h4><ul className="space-y-4"><li><a href="#" target="_blank" className="flex items-center gap-3 text-neutral-400 hover:text-pink-500 transition-colors"><Instagram size={20} /> <span>@conputodo</span></a></li><li><a href="#" target="_blank" className="flex items-center gap-3 text-neutral-400 hover:text-blue-500 transition-colors"><Facebook size={20} /> <span>Conputodo</span></a></li><li><a href="#" target="_blank" className="flex items-center gap-3 text-neutral-400 hover:text-white transition-colors"><Video size={20} /> <span>@conputodo</span></a></li></ul></div>
                    <div><h4 className="font-bold text-lg mb-6 border-b border-neutral-800 pb-2 inline-block">Gamer</h4><ul className="space-y-4"><li><a href="#" target="_blank" className="flex items-center gap-3 text-neutral-400 hover:text-purple-500 transition-colors"><Instagram size={20} /> <span>@conputodogamer</span></a></li><li><a href="#" target="_blank" className="flex items-center gap-3 text-neutral-400 hover:text-blue-600 transition-colors"><Facebook size={20} /> <span>Conputodo Gamer</span></a></li></ul></div>
                </div>
...
// ==========================================
// COMPONENTES AUXILIARES (DEFINIDOS COMPLETAMENTE)
// ==========================================

function FilterAccordion({ title, isOpen, onToggle, children }) {
    return (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <button onClick={onToggle} className="w-full p-4 flex items-center justify-between font-bold text-slate-900 hover:bg-gray-50 transition-colors">
                {title} {isOpen ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}
            </button>
            <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="p-4 pt-0 space-y-1">{children}</div>
            </div>
        </div>
    );
}
...
function LegalSection({ onBack }) { 
    return (
        <div className="container mx-auto px-4 py-16 max-w-4xl animate-in fade-in duration-500">
            <button onClick={onBack} className="mb-8 flex items-center gap-2 text-slate-500 hover:text-[#FF6600] transition-colors"><ArrowRight className="rotate-180" size={18}/> Volver al Inicio</button>
            <div className="bg-white p-12 rounded-2xl shadow-sm border border-gray-100 space-y-10">
                <div className="text-center border-b border-gray-100 pb-8">
                    <div className="w-16 h-16 bg-orange-50 text-[#FF6600] rounded-full flex items-center justify-center mx-auto mb-4"><ShieldCheck size={32}/></div>
                    <h1 className="text-3xl font-bold text-slate-900">Políticas y Garantía</h1>
                </div>
                <div className="space-y-4"><h3 className="text-xl font-bold">Garantía</h3><p className="text-slate-600">Garantía limitada por defectos de fábrica. No cubre daños eléctricos ni mal uso.</p></div>
                <div className="space-y-4"><h3 className="text-xl font-bold">Envíos</h3><p className="text-slate-600">Envíos nacionales por Zoom/Tealca. La mercancía viaja a riesgo del cliente.</p></div>
            </div>
        </div>
    ); 
}
...
function ContactSection({ db }) {
  ...
  const handleSubmit = async (e) => { e.preventDefault(); setStatus('loading'); try { await addDoc(collection(db, "wholesale_requests"), { ...formData, createdAt: serverTimestamp(), emailStatus: 'pending' }); setStatus('success'); setFormData({ companyName: '', companyRif: '', email: '', phone: '', message: '' }); } catch (error) { console.error(error); setStatus('error'); } };

  return (
    <div className="animate-in fade-in duration-500">
      <div className="bg-neutral-900 text-white py-20 text-center"><h1 className="text-4xl font-bold mb-4">Contáctanos</h1><p className="text-neutral-400">Maracay, Edo. Aragua</p></div>
      <div className="container mx-auto px-4 py-16 grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="space-y-8">
            <div className="space-y-6">
                <div className="flex items-start gap-4"><div className="p-3 bg-orange-100 text-[#FF6600] rounded-lg"><MapPin size={24}/></div><div><h4 className="font-bold text-slate-800">Ubicación</h4><p className="text-slate-600">CC Global, Maracay.</p><a href="https://maps.app.goo.gl/mZGVGx6C4LHT69ur6" target="_blank" className="text-sm text-[#FF6600] font-bold hover:underline flex items-center gap-1 mt-1">Ver en Google Maps <ArrowRight size={14} /></a></div></div>
            </div>
            <div className="w-full h-80 rounded-2xl overflow-hidden border border-gray-200 shadow-inner relative z-0">
               <MapContainer center={[10.249758418955139, -67.60040315522524]} zoom={17} scrollWheelZoom={false} style={{ height: "100%", width: "100%" }}><TileLayer attribution='&copy; OSM' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/><Marker position={[10.249758418955139, -67.60040315522524]}><Popup>Conputodo</Popup></Marker></MapContainer>
            </div>
        </div>
        <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-xl">
            {status === 'success' ? (<div className="bg-green-50 text-green-700 p-6 rounded-xl text-center"><Check size={24} className="mx-auto mb-2"/><h4 className="font-bold">¡Enviado!</h4><button onClick={() => setStatus('idle')} className="underline mt-2">Nuevo mensaje</button></div>) : (
              <form className="space-y-4" onSubmit={handleSubmit}>
                  <input type="text" name="companyName" value={formData.companyName} onChange={handleChange} className="w-full p-3 border rounded-lg" placeholder="Empresa" required/>
                  <input type="text" name="companyRif" value={formData.companyRif} onChange={handleChange} className="w-full p-3 border rounded-lg" placeholder="RIF" required/>
                  <input type="email" name="email" value={formData.email} onChange={handleChange} className="w-full p-3 border rounded-lg" placeholder="Correo" required/>
                  <input type="tel" name="phone" value={formData.phone} onChange={handleChange} className="w-full p-3 border rounded-lg" placeholder="Teléfono" required/>
                  <textarea name="message" value={formData.message} onChange={handleChange} className="w-full p-3 border rounded-lg" placeholder="Mensaje" rows="3" required></textarea>
                  <button disabled={status === 'loading'} className="w-full py-4 bg-slate-900 text-white font-bold rounded-lg hover:bg-[#FF6600] transition-colors">{status === 'loading' ? '...' : 'Enviar'}</button>
              </form>
            )}
        </div>
      </div>
...
function CartModal({ isOpen, cart, setCart, onClose, totalUSD, onRemove, exchangeRate }) {
  useEffect(() => { function handleKeyDown(e) { if (e.key === 'Escape' && isOpen) onClose(); } window.addEventListener('keydown', handleKeyDown); return () => window.removeEventListener('keydown', handleKeyDown); }, [isOpen, onClose]);
  const [step, setStep] = useState('cart'); const [loading, setLoading] = useState(false); const [client, setClient] = useState({ name: '', cedula: '', contact: '', method: 'pago_movil', shipping: 'tienda' }); const [confirmedOrder, setConfirmedOrder] = useState(null); 
  const handleSubmitOrder = async (e) => { e.preventDefault(); setLoading(true); try { const orderData = { clientName: client.name, clientCedula: client.cedula, clientContact: client.contact, shippingMethod: client.shipping, paymentMethod: client.method, products: cart.map(p => ({ id: p.id, title: p.title, qty: p.qty, price: p.prices.usd })), totalUsd: totalUSD, totalBs: totalUSD * exchangeRate, exchangeRateSnapshot: exchangeRate, status: 'pendiente', createdAt: serverTimestamp() }; const docRef = await addDoc(collection(db, "orders"), orderData); setConfirmedOrder({ ...orderData, id: docRef.id }); setStep('success'); setCart([]); } catch (error) { console.error(error); alert("Error al procesar"); } finally { setLoading(false); } };
  const handleWhatsappRedirect = () => { if (!confirmedOrder) return; const message = `Pedido #${confirmedOrder.id}`; window.open(`https://wa.me/584120000000?text=${encodeURIComponent(message)}`, '_blank'); };
  
  return (
    <div className={`fixed inset-0 z-50 bg-black/50 flex justify-end transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={onClose}>
      <div className={`w-full max-w-md bg-white h-full shadow-2xl flex flex-col transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`} onClick={(e) => e.stopPropagation()}>
        <div className="p-4 border-b flex justify-between items-center bg-slate-50"><h2 className="font-bold text-lg text-slate-800">{step === 'cart' ? 'Tu Carrito' : 'Finalizar'}</h2><button onClick={onClose}><X size={20}/></button></div>
        <div className="flex-1 overflow-y-auto p-4">
          {step === 'cart' && (cart.length === 0 ? <div className="text-center py-10 text-slate-400">Vacío</div> : cart.map(item => <div key={item.id} className="border-b py-2 flex justify-between"><span>{item.title} (x{item.qty})</span> <button onClick={() => onRemove(item.id)} className="text-red-500"><Trash2 size={16}/></button></div>))}
          {step === 'form' && (<form id="checkout-form" onSubmit={handleSubmitOrder} className="space-y-4"><input className="w-full border p-2 rounded" placeholder="Nombre" required onChange={e=>setClient({...client, name: e.target.value})}/><input className="w-full border p-2 rounded" placeholder="Cédula" required onChange={e=>setClient({...client, cedula: e.target.value})}/><input className="w-full border p-2 rounded" placeholder="WhatsApp" required onChange={e=>setClient({...client, contact: e.target.value})}/></form>)}
          {step === 'success' && <div className="text-center py-10"><h3 className="text-xl font-bold">¡Listo!</h3><button onClick={handleWhatsappRedirect} className="bg-green-500 text-white px-4 py-2 rounded mt-4">Finalizar en WhatsApp</button></div>}
        </div>
        {step !== 'success' && cart.length > 0 && <div className="p-4 border-t"><div className="flex justify-between font-bold text-xl mb-4"><span>Total:</span><span>${totalUSD.toFixed(2)}</span></div>{step === 'cart' ? <button onClick={() => setStep('form')} className="w-full bg-[#FF6600] text-white py-3 rounded-xl font-bold">Continuar</button> : <button form="checkout-form" disabled={loading} className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold">{loading ? '...' : 'Confirmar'}</button>}</div>}
      </div>
    </div>
  );
}

function FloatingWhatsapp() { return (<a href={`https://wa.me/${COMPANY_PHONE}`} target="_blank" className="fixed bottom-6 right-6 z-50 bg-[#25D366] text-white p-4 rounded-full shadow-xl hover:scale-110 transition-transform"><MessageCircle size={32}/></a>); }
function CookieBanner({ onAccept }) { return (<div className="fixed bottom-0 w-full bg-neutral-900 text-white p-4 flex justify-between items-center z-50 shadow-lg"><p className="text-sm">Usamos cookies para mejorar tu experiencia.</p><button onClick={onAccept} className="bg-[#FF6600] px-4 py-1 rounded text-sm font-bold">Aceptar</button></div>); }